...
function renderComment(findings: Finding[], durationMs: number): string {
  const lines: string[] = [COMMENT_MARKER, "## CodeMoat scan results", ""];

  if (findings.length === 0) {
    lines.push("No issues found in the changed files.");
  } else {
    const bySeverity = SEVERITY_ORDER.map((s) => [s, findings.filter((f) => f.severity === s).length] as const).filter(
      ([, count]) => count > 0
    );
    lines.push(
    ...
    for (const f of findings) {
      ...
      lines.push(
        `| ${SEVERITY_EMOJI[f.severity]} ${f.severity} | \`${f.file}\` | ${f.line} | \`${f.ruleId}\` | ${f.cwe ?? "-"} | ${desc} | ${fix} |`
      );
    }
  }
...
async function main(): Promise<void> {
  ...
  if (token) {
    ...
    await upsertComment(octokit, owner, repo, pr.number as number, body);
  } else {
  ...
  if (apiKey) {
    ...
    await reportScan(result, {
      ...
      commitSha: (pr.head as { sha: string }).sha,
  ...
  if (failOnSeverity !== "none") {
    const threshold = SEVERITY_RANK[failOnSeverity as Finding["severity"]];
