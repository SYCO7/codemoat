...
async function resolveGitTargets(cwd, base, all) {
    if (all) {
...
function printFindings(findings, cwd) {
...
async function runScanCommand(argv) {
    ...
    const { targets, baseRef } = await resolveGitTargets(cwd, values.base, Boolean(values.all));
    if (targets.length === 0) {
